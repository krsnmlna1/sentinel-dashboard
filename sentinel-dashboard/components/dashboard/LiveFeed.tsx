...
export function LiveFeed() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const formattedProtocols = result.data.slice(0, 5).map((p: any) => ({
          ...p,
          tvl: typeof p.tvl === 'number' 
  ...
  // Convert TVL string to number for display
  const parseTVL = (tvl: string) => {
    const num = parseFloat(tvl.replace(/[$,M]/g, ''));
  ...
  return (
    <div className="h-full flex flex-col bg-sentinel-card rounded-xl border border-white/5 overflow-hidden">
      ...
      {/* Error Message */}
      {error && (
        <div className="p-3 bg-sentinel-red/10 border-b border-sentinel-red/20 text-sentinel-red text-xs">
          ⚠️ {error}
        </div>
      )}
            ...
            {protocols.map((protocol, index) => (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-white group-hover:text-sentinel-blue transition-colors flex items-center gap-2">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleCopy(protocol.slug, protocol.id);
                          }}
                          className="text-gray-400 hover:text-white transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100 flex items-center justify-center w-5 h-5 rounded hover:bg-white/10"
                          title="Copy Slug"
                        >
                  ...
                  <div className={`px-3 py-1 rounded border text-xs font-bold font-mono ${getRiskColor(protocol.score)}`}>
                    RISK: {protocol.score}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <div className="text-[10px] text-gray-400 mb-1">TVL</div>
                    <div className="text-sm font-mono text-white">
                      {protocol.tvl}
                    </div>
                  </div>
                  <div>
                    <div className="text-[10px] text-gray-400 mb-1">7D CHANGE</div>
                    <div className={`text-sm font-mono ${protocol.change < 0 ? 'text-sentinel-red' : 'text-sentinel-green'}`}>
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <button 
                    onClick={() => window.open(`/auditor?contract=${protocol.slug}`, '_blank')}
                    className="flex-1 flex items-center justify-center gap-2 py-1.5 rounded bg-sentinel-red/10 text-sentinel-red text-xs hover:bg-sentinel-red/20 transition-colors border border-sentinel-red/20"
                  >
                    <Shield size={12} />
                    QUICK AUDIT
                  </button>
                  {protocol.url && (
                    <button 
                      ...
                      className="flex-1 flex items-center justify-center gap-2 py-1.5 rounded bg-sentinel-blue/10 text-sentinel-blue text-xs hover:bg-sentinel-blue/20 transition-colors border border-sentinel-blue/20"
                    >
      ...
      <div className="p-3 border-t border-white/5 bg-white/[0.02]">
        <Link href="/targets" className="w-full py-2 text-xs text-gray-400 hover:text-white flex items-center justify-center gap-2 transition-colors">
